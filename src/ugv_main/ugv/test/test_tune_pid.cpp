...
TEST(TunePID, TestUgvCollectorMotorSpeed) {
    ...
    std::map<int, Command> commands;
    //commands[0] = CmdVel{0.0, 0.0};
    commands[0] = CmdPID{25.0, 100.0, 0.0, 0.0};
...
TEST(TunePID, TestUgvCollectorMotorSpeedFloor) {
    ...
    // set up command sequence
    // these are time sampling targets in seconds
    std::vector<double> targets = {5.75, 7.75};
    std::map<int, Command> commands;
    //commands[0] = CmdVel{0.0, 0.0};
    commands[0] = CmdPID{40.0, 300.0, 0.0, 0.0};
    commands[1] = CmdFF{175.0, 10.0};
    commands[3] = CmdVel{0.3, 0.2};
    commands[4] = CmdVel{0.3, 0.2};
    commands[5] = CmdVel{-0.3, -0.2};
    commands[6] = CmdVel{-0.3, -0.2};
    commands[7] = CmdVel{0.0, 0.0};
    int argc = 0;
    char ** argv = nullptr;
    rclcpp::init(argc, argv);
    auto node = std::make_shared<UgvCollector>(commands);
